...
WHERE 			GLP.PERIOD_SET_NAME            	= GL.PERIOD_SET_NAME		--JOIN para asociar GL a GLP. 
		AND 	GL.LEDGER_ID                  	= :P_LEDGER_ID
		AND 	TRUNC(GLP.Start_Date)         	<= TRUNC(SYSDATE)			--Tambien poner esto ya que hay periodos de 1 año superior (por ej. ahora es 2020 pero hay periodos cargados de 2021).
